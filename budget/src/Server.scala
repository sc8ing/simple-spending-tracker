...
case class HttpServer(
  ...
  def encodeLoginJwt(forUser: String): Task[String] =
    logInfo("Encoding login JWT for user " + forUser) *>
   ...
   val login: http.App[Any] = Http.collectZIO[Request] {
     case req @ Method.POST -> Root / "login" =>
       for {
         ...
         _ <- logInfo(s"login form attempted with values $form")
         user <- fromOption(form.get("username").flatMap(_.stringValue)) | Status.BadRequest.toResponse
         pass <- fromOption(form.get("password").flatMap(_.stringValue)) | Status.BadRequest.toResponse
         authed <- auth.validateUserPass(user, pass).logError | Status.InternalServerError.toResponse
         res <- if (!authed)
                  succeed(Status.Forbidden.toResponse)
                else
  ...
  def api(creds: AuthedCreds): HttpApp[Any, Nothing] =  Http.collectZIO[Request] {
    case req @ Method.GET -> Root / "api" / "transaction" =>
      succeed(Response(body = Body.fromCharSequence("main page")))
  }
    ...
    login ++ requireAuth(
      ...
      Http.fromHttpZIO[(Request, JwtClaim)]((req, claim) =>
        ...
        fromEither(claim.content.fromJson[JwtContent])
          ...
          .tap(jwt => logInfo(s"Got request with JWT $jwt"))
          .map { jwtContent =>
            ...
            val updateJwtHandler = Handler.fromFunctionZIO[Response](res => for {
              ...
              resNew <- if (!isExpired)
                          ...
                          logInfo(s"Making new JWT for ${jwtContent.username}") *>
  ...
  val runServer = Server.serve(server).provide(Server.live, serverConfig)
}
object HttpServer:
  val liveLayer = ZLayer.fromFunction(HttpServer(_, _, _))
